            ...
            if commence_time:
                ...
                try:
                    ...
                    if original_time.tzinfo is None:
                        original_time = original_time.replace(tzinfo=datetime.now().astimezone().tzinfo)
                    
                    current_time = datetime.now(original_time.tzinfo)
                    
                    # If the original time is in the future, use it as is
                    if original_time > current_time:
                        # UFC events are almost always on Saturdays, so adjust the date if needed
                        from datetime import timedelta
                        
                        # Get the day of week (0=Monday, 6=Sunday in Python)
                        day_of_week = original_time.weekday()
                        
                        # If it's not Saturday (5), adjust to the next Saturday
                        if day_of_week != 5:  # 5 = Saturday
                            days_until_saturday = (5 - day_of_week) % 7
                            if days_until_saturday == 0:
                                days_until_saturday = 7  # Next Saturday
                            adjusted_time = original_time + timedelta(days=days_until_saturday)
                            logger.info(f"Adjusted event time from {original_time.strftime('%A, %Y-%m-%d %H:%M')} to Saturday: {adjusted_time.strftime('%A, %Y-%m-%d %H:%M')}")
                            event_start_time = adjusted_time.isoformat()
                        else:
                            event_start_time = original_time.isoformat()
                        
                        logger.info(f"Using original event start time from odds API: {event_start_time}")
                    else:
                        # If odds time is in the past, we can't reliably determine event time
                        # UFC Stats doesn't provide event start times, so we'll skip this event
                        logger.warning(f"Odds API time is in the past ({original_time}) and no reliable event time available")
                        event_start_time = None
                
                except Exception as e:
                    logger.warning(f"Error parsing commence time '{commence_time}': {e}")
                    # Can't determine event time reliably
                    event_start_time = None
                    logger.info(f"No reliable event start time available")
